...
export default function PremiumCarsSlider({ cars, loading = false }) {
  ...
  useEffect(() => {
    const updateCards = () => {
      ...
      if (width < 640)
        setCardsToShow(1); // Mobile → 1 big card
      else if (width < 1024)
        setCardsToShow(1); // Tablet → 1 big card
      else if (width < 1280)
        setCardsToShow(2); // Small laptop → 2 cards
  ...
  // Auto-play
  useEffect(() => {
    if (isHovered || cars.length <= cardsToShow || loading || showSkeleton)
      return;
  ...
  const handleTouchEnd = () => {
    if (!touchStartX.current || !touchEndX.current || loading || showSkeleton)
      return;
      ...
      >
        ...
        <div className="overflow-hidden">
          <motion.div
            ...
            style={{
              width: `${cars.length * cardWidth + (cars.length - 1) * gap}px`,
            }}
          >
            {cars.map((car) => (
              <div
